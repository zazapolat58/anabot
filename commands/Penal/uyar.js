    ...
    async run(message, args, perm) {
        ...
        uyarılar.findOne({user: user.id}, async(err,res) => {
            if(!res) {
                ...
                message.channel.send(`<a:tik:851064010497720350> ${user} kişisine **${sebep}** sebebiyle ilk uyarısı verildi.Kullanıcının ceza puanı \`${await this.client.punishPoint(user.id)}\` oldu.`)
                ...
                if(res.uyarılar.length == 2) {
                    message.channel.send(`<a:tik:851064010497720350> ${user} kişisine **${sebep}** sebebiyle 2. uyarısı verildi.Kullanıcının ceza puanı \`${await this.client.punishPoint(user.id)}\` oldu.`)
                    user.roles.remove(Role.Warn.Role)
                    user.roles.add(Role.Warn.Role2)
                }
                if(res.uyarılar.length == 3) {
                    message.channel.send(`<a:tik:851064010497720350> ${user} kişisine **${sebep}** sebebiyle 3. uyarısı verildi.Kullanıcının ceza puanı \`${await this.client.punishPoint(user.id)}\` oldu.`)
