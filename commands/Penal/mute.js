...
class Mute extends Command {
    ...
    async run(message, args, perm) {
        ...
        let cıkaralım = time + Date.parse(new Date(new Date().getTime()+ (1000*60*60*3)));
        let şuanki = moment(Date.now() + (1000*60*60*3)).format("LLL");
      ...
      //  let puan = await this.client.punishPoint(user.id)
        ...
        const mutelendı = new Discord.MessageEmbed()
            ...
            .setDescription(`${user} (\`${user.user.tag}\` - \`${user.id}\`) kişisi **__${await this.client.turkishDate(muteTime)}__** boyunca metin kanallarında susturuldu\n\n• Susturulma sebebi: \`${args.slice(2).join(" ")}\`\n• Chat Mute atılma tarihi: \`${şuanki}\`\n• Chat Mute bitiş tarihi: \`${sonraki}\``)
        ...
        await mutes.findOne({ user: user.id }, async (err, doc) => {
            const newMute = new mutes({
                ...
                endDate: (Date.now() + (1000*60*60*3)) + muteTime,
        ...
        await data.find({}).sort({ ihlal: "descending" }).exec(async (err, res) => {
            const newData = new data({
                ...
                bitiş: moment((Date.now() + (1000*60*60*3)) + time).format("LLL")
            })
